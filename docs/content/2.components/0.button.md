...
::Alert{type="warning"}
If you are using `to` prop on RButton in a nuxt application, it's prefered to pass `NuxtLink` directly as the `tag` prop.
```vue
<RButton :tag="NuxtLink" to="/some-route" />
```
otherwise it will use the global `defineNuxtLink` method to create a `NuxtLink`. if you have auto imports disabled, then you should pass this function directly to `RButton`.
```ts
<RButton :defineNuxtLink="defineNuxtLink" to="/some-route" />
