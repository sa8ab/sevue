...
## Creating Options
You can allow user to create new option. to do so you need to add `canCreateOption` to `RSelect` and use the `newOption` event that will be called whenever user has something typed on search input. the parameter of event is
```ts
{newOption: string, isAlreadyInValue: boolean, isAlreadyInOptions: boolean}
```
...
| ---------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| setSelectedItems | -                                                           | Function for setting selected items (usefull when items are not available but there are some values selected) |
| rInput           | `InstanceType\<typeof RInput>`                              | The input of `RSelect` (can be usefull for calling methods of `RInput`).                                      |
